...
addCelllineLossOfY <- function() {
  ...
  perc <- data.all %>%
    ...
    summarise_at(vars(starts_with("chry")), quantile, probs = 0.99, na.rm = TRUE) %>%
    unlist()
  
  # -------- chry_avg_counts should not be taken as a filter, because this is dependent on the sequencing depth
  data.loy <- data.all %>%
    mutate(lossofy = ifelse(sex == "female" | is.na(sex), NA, 
  ...
  final_data <- data.loy %>% select(celllinename, lossofy) %>% filter(!is.na(lossofy))
