...
getCelllineMetMap <- function() {
  ...
  readMetmapExcel <- function(filepath) {
    ...
    for (s in readxl::excel_sheets(filepath)) {
      data2 <- readxl::read_xlsx(filepath, sheet = s) %>%
        ...
        dplyr::mutate(organ = gsub("(metp500\\.|5)", "", s))
      if (nrow(data) == 0) {
        data <- data2
      } else {
        data <- data %>% dplyr::bind_rows(data2)
      }
    }
    data
  }
  ...
  dataDB <- data %>%
    dplyr::select(celllinename, organ, met_potential = mean, ci5percent = CI.05, ci95percent = CI.95, penetrance)
